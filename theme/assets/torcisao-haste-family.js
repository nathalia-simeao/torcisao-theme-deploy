...
function hfPageLanguage(){const htmlLang=(document.documentElement.lang||'').toLowerCase(),path=location.pathname.toLowerCase();if(htmlLang.startsWith('en')||path.startsWith('/en/'))return'en';if(htmlLang.startsWith('es')||path.startsWith('/es/'))return'es';return'pt';}
const hfFormTranslations={
 en:[
  ['Nome','Name'],['Empresa','Company'],['E-mail Corporativo','Corporate Email'],['E-mail','Email'],['Contato / WhatsApp','Phone / WhatsApp'],['O que deseja cotar?','What would you like to quote?'],
  ['Baixa Camada','Low Coating'],['Alta Camada','High Coating'],['Conectores','Connectors'],
  ['Diâmetro - Haste Baixa Camada','Diameter - Low Coating Rod'],['Comprimento - Haste Baixa Camada','Length - Low Coating Rod'],['Quantidade de Peças - Baixa Camada','Quantity - Low Coating Rod'],
  ['Diâmetro - Haste Alta Camada','Diameter - High Coating Rod'],['Comprimento - Haste Alta Camada','Length - High Coating Rod'],['Quantidade de Peças - Alta Camada','Quantity - High Coating Rod'],
  ['Modelo do Conector','Connector Model'],['Quantidade de Peças - Conectores','Quantity - Connectors'],['Sob Consulta','Upon Request'],
  ...
  ['Eu li e concordo com a','I have read and agree to the'],['da Torcisão Trefilados e autorizo o processamento dos meus dados.','of Torcisão Trefilados and authorize the processing of my data.'],
 ...
 es:[
  ['Nome','Nombre'],['Empresa','Empresa'],['E-mail Corporativo','Correo electrónico corporativo'],['E-mail','Correo electrónico'],['Contato / WhatsApp','Teléfono / WhatsApp'],['O que deseja cotar?','¿Qué desea cotizar?'],
  ['Baixa Camada','Baja Capa'],['Alta Camada','Alta Capa'],['Conectores','Conectores'],
  ['Diâmetro - Haste Baixa Camada','Diámetro - Varilla de Baja Capa'],['Comprimento - Haste Baixa Camada','Longitud - Varilla de Baja Capa'],['Quantidade de Peças - Baixa Camada','Cantidad de piezas - Baja Capa'],
  ['Diâmetro - Haste Alta Camada','Diámetro - Varilla de Alta Capa'],['Comprimento - Haste Alta Camada','Longitud - Varilla de Alta Capa'],['Quantidade de Peças - Alta Camada','Cantidad de piezas - Alta Capa'],
  ['Modelo do Conector','Modelo del conector'],['Quantidade de Peças - Conectores','Cantidad de piezas - Conectores'],['Sob Consulta','Bajo consulta'],
  ...
  ['Eu li e concordo com a','He leído y acepto la'],['da Torcisão Trefilados e autorizo o processamento dos meus dados.','de Torcisão Trefilados y autorizo el tratamiento de mis datos.'],
  ['Política de Cookies','Política de Cookies'],['Política de Privacidade','Política de Privacidad'],['Enviar','Enviar'],
  ['Preencha este campo obrigatório.','Complete este campo obligatorio.'],['Selecione pelo menos uma opção.','Seleccione al menos una opción.'],['Insira um endereço de e-mail válido.','Introduzca una dirección de correo electrónico válida.'],['Insira um número de telefone válido.','Introduzca un número de teléfono válido.']
 ]
};
function hfHubSpotI18n(){
 const lang=hfPageLanguage();
 if(lang==='en')return{locale:'en',submitText:'Submit',translations:{en:{fieldLabels:{firstname:'Name',company:'Company',email:'Email',phone:'Phone / WhatsApp',o_que_deseja_cotar:'What would you like to quote?',diametro__haste_baixa_camada:'Diameter - Low Coating Rod',comprimento___haste_baixa_camada:'Length - Low Coating Rod',quantidade_de_pecas:'Quantity - Low Coating Rod',diametro__haste_alta_camada:'Diameter - High Coating Rod',comprimento__haste_alta_camada:'Length - High Coating Rod',quantidade_de_pecas__alta_camada:'Quantity - High Coating Rod',modelo_do_conector:'Connector Model',quantidade_de_pecas__conectores:'Quantity - Connectors'},submitText:'Submit'}}};
 if(lang==='es')return{locale:'es',submitText:'Enviar',translations:{es:{fieldLabels:{firstname:'Nombre',company:'Empresa',email:'Correo electrónico',phone:'Teléfono / WhatsApp',o_que_deseja_cotar:'¿Qué desea cotizar?',diametro__haste_baixa_camada:'Diámetro - Varilla de Baja Capa',comprimento___haste_baixa_camada:'Longitud - Varilla de Baja Capa',quantidade_de_pecas:'Cantidad de piezas - Baja Capa',diametro__haste_alta_camada:'Diámetro - Varilla de Alta Capa',comprimento__haste_alta_camada:'Longitud - Varilla de Alta Capa',quantidade_de_pecas__alta_camada:'Cantidad de piezas - Alta Capa',modelo_do_conector:'Modelo del conector',quantidade_de_pecas__conectores:'Cantidad de piezas - Conectores'},submitText:'Enviar'}}};
 return{locale:'pt-br',submitText:'Enviar'};
}
function translateHubSpotForm(node){const lang=hfPageLanguage();if(lang==='pt'||!node)return;const pairs=hfFormTranslations[lang]||[];const walker=document.createTreeWalker(node,NodeFilter.SHOW_TEXT);let textNode;while((textNode=walker.nextNode())){const raw=textNode.nodeValue||'',trimmed=raw.trim();if(!trimmed)continue;let next=trimmed;for(const [from,to] of pairs){if(next===from){next=to;break}if(from.length>18&&next.includes(from))next=next.split(from).join(to);}if(next!==trimmed){const lead=(raw.match(/^\s*/)||[''])[0],trail=(raw.match(/\s*$/)||[''])[0];textNode.nodeValue=lead+next+trail;}}}
function watchHubSpotForm(node){translateHubSpotForm(node);let scheduled=false;const observer=new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;translateHubSpotForm(node);});});observer.observe(node,{childList:true,subtree:true,characterData:true});setTimeout(()=>translateHubSpotForm(node),150);setTimeout(()=>translateHubSpotForm(node),600);}

const drawer=$('hfQuoteDrawer'),tab=$('hfQuoteTab'),form=$('hfQuoteForm');let mountedFormId='';function label(open){const s=q('.btn-cta-mobile-label',tab);if(s)s.textContent=open?'Fechar':'Solicitar cotação';tab.setAttribute('aria-label',open?'Fechar cotação':'Solicitar cotação')}function currentFormId(){return cfg.formIds?.[kind]||cfg.formIds?.baixa||'8fdff701-c5a7-4684-9358-d557a70425a5'}function currentOrigin(){return kind==='alta'?'LP Haste AC':kind==='conectores'?'LP Haste | Conectores':'LP Haste BC'}function mountForm(force=false){const wanted=currentFormId();if(!force&&mountedFormId===wanted&&form.querySelector('form,iframe'))return;mountedFormId=wanted;form.innerHTML='<div class="hf-form-loading">Preparando formulário…</div>';const boot=()=>{if(window.hbspt?.forms){form.innerHTML='';const i18n=hfHubSpotI18n();window.hbspt.forms.create(Object.assign({portalId:cfg.portalId||'50818463',formId:wanted,region:'na1',target:'#hfQuoteForm',onFormReady:function(el){const node=el instanceof Element?el:(el&&el[0])?el[0]:form.querySelector('form.hs-form');const input=node?.querySelector('input[name="pagina_de_origem_do_lead"]');if(input){input.value=currentOrigin();input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));}if(node)watchHubSpotForm(node);},onFormSubmitted:function(){track('generate_lead',{origin:'haste_quote_drawer',product:'haste_aterramento',variant:kind})}},i18n));return}setTimeout(boot,100)};if(!document.querySelector('script[src*="js.hsforms.net/forms/embed/v2.js"]')){const s=document.createElement('script');s.src='https://js.hsforms.net/forms/embed/v2.js';s.charset='utf-8';s.onload=boot;document.head.appendChild(s)}else boot()}function refreshQuoteForm(){if(drawer.classList.contains('is-open'))mountForm(true)}function openDrawer(origin='haste_aba_cotacao'){drawer.classList.add('is-open');drawer.setAttribute('aria-hidden','false');document.body.classList.add('torcisao-form-drawer-open');label(true);mountForm();track('quote_form_open',{origin,product:'haste_aterramento',variant:kind})}function closeDrawer(){drawer.classList.remove('is-open');drawer.setAttribute('aria-hidden','true');document.body.classList.remove('torcisao-form-drawer-open');label(false)}tab.addEventListener('click',()=>drawer.classList.contains('is-open')?closeDrawer():openDrawer(tab.dataset.analyticsOrigin));q('.hf-quote-close',drawer)?.addEventListener('click',closeDrawer);qa('.js-hf-quote').forEach(b=>b.addEventListener('click',()=>openDrawer(b.dataset.analyticsOrigin||'haste_cta')));
