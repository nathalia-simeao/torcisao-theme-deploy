...
const data={
 btc:{title:'Baixo Teor de Carbono · 1004 a 1020',lead:'Barra trefilada de baixo teor de carbono, em perfil redondo, com controle dimensional e acabamento obtidos por processo a frio.',image:'assets/barra1.webp',facts:[['Faixa de aço','1004 a 1020'],['Bitola','2,00 a 15,88 mm'],['Perfil','Redondo'],['Tolerância','Sob consulta']],specs:[['Bitola / diâmetro','2,00 a 15,88 mm'],['Tolerância','Sob consulta'],['Acondicionamento','Feixes embalados'],['Perfil','Redondo'],['Acabamento','Trefilado ou trefilado polido']],availability:[['Faixa de aço','1004 a 1020'],['Fornecimento','Feixes embalados'],['Comprimento','Conforme especificação / consulta']],apps:['Fixadores','Autopeças','Cesto metálico','Rack metálico','Molas helicoidais','Amortecedores']},
...
function setHubSpotField(form,name,value){
 const fields=qa(`[name="${name}"]`,form);if(!fields.length)return false;
 let applied=false;
 fields.forEach(field=>{
  if(field.type==='radio'||field.type==='checkbox'){
   const should=field.value===value;
   if(should){field.checked=true;field.dispatchEvent(new Event('input',{bubbles:true}));field.dispatchEvent(new Event('change',{bubbles:true}));applied=true;}
   return;
  }
...
function syncBarQuoteForm(formNode){
 const form=formNode||q('#bfQuoteForm form.hs-form')||q('#bfQuoteForm form');if(!form)return;
 const selected=formVariantMap[kind]||formVariantMap.btc;
 setHubSpotField(form,'barra_trefilada',selected.barra_trefilada);
