 ...
 * Nenhum controlador legado de imagem roda aqui. */
...
$('thToleranceRun')?.addEventListener('click',()=>{const input=$('thToleranceDiameter'),out=$('thToleranceResult');const d=parseFloat(String(input?.value||'').replace(',','.'));if(!out)return;if(!Number.isFinite(d)||d<=1||d>250){out.innerHTML='<div class="th-empty-result"><span>BITOLA</span><p>Informe uma bitola válida acima de 1 mm e até 250 mm</p></div>';return;}const row=toleranceRows.find(r=>d>r.min&&d<=r.max);if(!row){out.innerHTML='<div class="th-empty-result"><span>CONSULTA</span><p>Não encontrei uma faixa para esta bitola. Consulte nosso consultor</p></div>';return;}const h9=d<9.53?null:row.h9;const card=(cls,tol)=>tol==null?`<div><small>${cls}</small><strong>Sob consulta</strong></div>`:`<div><small>${cls}</small><strong>${fmtTol(tol)}</strong></div>`;out.innerHTML=`<div class="th-tolerance-values">${card('h9',h9)}${card('h10',row.h10)}${card('h11',row.h11)}</div><p class="th-tolerance-range-note">Trefilado · perfil redondo · bitola consultada: ${d.toLocaleString('pt-BR',{maximumFractionDigits:2})} mm</p>`;toleranceSummary=[`Bitola: ${d.toLocaleString('pt-BR',{maximumFractionDigits:2})} mm`,`h9: ${fmtTol(h9)}`,`h10: ${fmtTol(row.h10)}`,`h11: ${fmtTol(row.h11)}`];window.TorcisaoI18n?.refresh?.();track('technical_tool_calculate',{tool:'tolerancia_dimensional',diameter_mm:d});});
