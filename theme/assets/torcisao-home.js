...
const manualData={arame:{name:'Arame Trefilado',text:'Baixo, Médio e Alto Teor de Carbono organizados na página de Arame Trefilado',url:cfg.urls?.arame||'/arame-trefilado/'},barra:{name:'Barra Trefilada',text:'Baixo, Médio e Alto Teor de Carbono e Ressulfurado organizados na página de Barra Trefilada',url:cfg.urls?.barra||'/barra-trefilada/'},haste:{name:'Haste de Aterramento',text:'Baixa Camada, Alta Camada e Conectores ficam dentro da página unificada de Haste de Aterramento',url:cfg.urls?.haste||'/haste-de-aterramento/'}};
...
$('thAssistantOpen')?.addEventListener('click',openAssistant);$('thAssistantOpenBottom')?.addEventListener('click',openAssistant);
qa('#ferramentas-tecnicas .th-tool-card').forEach((card,index)=>card.addEventListener('click',e=>{
  if(e.target.closest('a,button,input,select,textarea'))return;
  (index===0?$('thToleranceOpen'):$('thAssistantOpen'))?.click();
}));qa('[data-th-assistant-close]').forEach(b=>b.addEventListener('click',closeAssistant));assistant?.addEventListener('click',e=>{if(e.target===assistant)closeAssistant()});
